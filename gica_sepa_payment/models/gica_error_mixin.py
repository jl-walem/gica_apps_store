...
class GicaSepaTranslationMixin(models.AbstractModel):
    _name = 'gica.sepa.translation.mixin'
    _description = 'GICA SEPA Translation Mixin'
    ...
    def _gica_error(self, message, *args):
        translated = self._gica_translate_po(message)

        if args:
            translated = translated % args

        raise UserError(translated)
